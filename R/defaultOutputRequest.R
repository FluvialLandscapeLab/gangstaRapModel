...
#'@export
defaultOutputWithIsotopes <- function(gangstas, isotopesToTrack){
  # Make pool values data frame
  pools <-  subsetGangstas(gangstas, "class", gangsta:::gangstaClassName("pool"))
  poolNames <-  getGangstaAttribute(pools, gangsta:::gangstaAttributeName("name"))
  elementName <-  getGangstaAttribute(pools, gangsta:::gangstaAttributeName("element"))
  ...
  atomicFractions <- list(
    pool = unlist(lapply(poolNamesWithIsotopeTracking, 
                         function(poolName, isotopesToTrack) rep(poolName, times = length(isotopesToTrack[[substr(poolName, nchar(poolName), nchar(poolName))]])),
                         isotopesToTrack = isotopesToTrack)),
    isotope = unlist(lapply(poolNamesWithIsotopeTracking, 
                            function(poolName, isotopesToTrack) isotopesToTrack[[substr(poolName, nchar(poolName), nchar(poolName))]],
                            isotopesToTrack = isotopesToTrack)),
    fraction.initial = unlist(lapply(poolNamesWithIsotopeTracking, 
                                     function(poolName, isotopesToTrack) {
                                       atomicMasses <- isotopesToTrack[[substr(poolName, nchar(poolName), nchar(poolName))]]
                                       lapply(atomicMasses, function(atomicMass) {
                                         parse(text = 
                                                 paste0(
                                                   poolName, '.initialAF["',atomicMass,'"]'
                                                 )
                                         )
                                       }
                                       )
                                     ...
                                     isotopesToTrack = isotopesToTrack),
                              recursive = F),
    fraction.final = unlist(lapply(poolNamesWithIsotopeTracking, 
                                     function(poolName, isotopesToTrack) {
                                       atomicMasses <- isotopesToTrack[[substr(poolName, nchar(poolName), nchar(poolName))]]
                                       lapply(atomicMasses, function(atomicMass) {
                                         parse(text = 
                                                 paste0(
                                                   poolName, '.finalAF["',atomicMass,'"]' 
                                                 )
                                         )
                                       }
                                       )
  ...
  deltaValues <- list(
    pool = poolNamesWithIsotopeTracking,
    deltaValue.initial = lapply(poolNamesWithIsotopeTracking, 
                                function(poolName) {
                                  parse(text = 
                                          paste0(
                                            poolName, ".initialDelta"
                                          )
                                  )
                                }
    ),
    deltaValue.final = lapply(poolNamesWithIsotopeTracking, 
                              function(poolName) {
                                parse(text = 
                                        paste0(
                                          poolName, ".finalDelta"
  ...
  outputRequestList <- list(
    ...
    processEnergyVals = processEnergyVals,
    respirationEnergyVals = respirationEnergyVals,
    atomicFractions = atomicFractions,
